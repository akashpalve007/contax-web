...
<div class="container flex items-center py-3 mb-4 border-b-2 xl:hidden">
    ...
    <div class="flex items-center m-auto">
        <img src="{{ asset('public/img/logo.jpeg') }}" class="w-24 m-auto" alt="Akaunting" />
...
>
    ...
    <nav class="menu-list js-main-menu" id="sidenav-main">
        ...
        <div class="relative mb-2 cursor-pointer flex justify-center">
            <button type="button" class="flex items-center" data-dropdown-toggle="dropdown-menu-company">
                <div class="w-8 h-8 flex items-center justify-center">
                    <img src="{{ asset('public/img/logo.jpeg') }}" class="w-24 " alt="Akaunting" />
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('div').forEach(function(div) {
        div.childNodes.forEach(function(node) {
            if (node.nodeType === 3 && node.nodeValue.includes('Purchases')) {
               // console.log('Original node value:', node.nodeValue); // Optional: for debugging
                node.nodeValue = node.nodeValue.replace('Purchases', 'Expenses');
                //console.log('Updated node value:', node.nodeValue); // Optional: for debugging
            }
        }); 
    });
});
...
<!-- <script>
    window.onload = function() {
        // Get the span element with the specified class and attributes, and its child with the class material-icons-outlined
        var spanElement = document.querySelector('.w-8.h-8.flex.items-center.justify-center.ltr\\:mr-2.rtl\\:ml-2.pointer-events-none .material-icons-outlined');

        // Set the margin-left property dynamically
